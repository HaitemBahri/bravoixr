...
- `components/` — for elements daisyUI doesn't cover (`page`, `text`, `icon`, and the still-unmigrated `card`), plus small gap-override rules against daisyUI's own selectors for migrated components (currently: `button`'s focus-visible ring; `input`/`toggle` need none — both are consumed as pure daisyUI, no bravoixr override at all). `card`'s own base class is also a gap-override on daisyUI's `.card` (trimmed to just padding/background/color), but its anatomy classes (`card-title`, `card-description`, `card-header`, `card-footer`, `card-media`) have no daisyUI equivalent and stay full bravoixr implementations. Consumes semantic tokens only.
