...
- **A prop-driven variant reaches the CSS through a `data-` attribute, not a class.** A class on a rendered element is a public selector a consumer can target, which the first rule forbids. `<div data-axis={axis}>` with `div[data-axis="column"] { … }` compiles to `div[data-astro-cid-…][data-axis="column"]` — the rule stays in the component's own `<style>` and stays scoped to it. Where the value is arbitrary rather than one of a few known states, set a custom property on the root element instead — `<div style={`--swatch-color: ${color}`}>`, read as `var(--swatch-color)`. Prefer that to Astro's `define:vars`, which stamps the same declaration onto *every* element in the template; custom properties inherit, so the root alone is enough.
...
- **The preview is off-limits.** Never add anything to `apps/preview-astro` — any section, any page — without an explicit instruction to do so. Building a component is not permission to demo it; a new component ships on its own and the user decides if, when and where it appears in the preview.
