...
Five measurable categories carry through `identity/`, `primitives/`, and `semantics/` as same-named files: **color, typography, layout, motion, icons**. A sixth, **feel** (density, taste rules, base-theme choice), is `identity/`-only — it has no token file.
